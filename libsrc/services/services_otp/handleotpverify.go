...
func HandleOTPVerify(reqBrokerDataMap map[string]interface{}) int {
	var respBuffer []byte
	...
	_, ok = otpObjMap["otp"]
	if !ok {
		appErrDesc := fmt.Sprintf("otp does not exist in otp_obj", "otp_obj")
		correctiveAction := fmt.Sprintf("Ensure that otp does exist in otp_obj")
		reqBrokerDataMap[reqbrokerdef.ReqBrokerDataMapRespObj] = msgutil.SetResp(rejectdef.LMS_Admin_Service_Reject_HandleSendOTPErr, []byte(appErrDesc), []byte(correctiveAction))
		return -1
	}
	if !datatypeutil.IsString(otpObjMap["otp"]) {
		appErrDesc := fmt.Sprintf("otp[%T] is not string in reqBrokerDataMap", otpObjMap["otp"])
		correctiveAction := fmt.Sprintf("Ensure that otp[%T] is a string in reqBrokerDataMap", otpObjMap["otp"])
		reqBrokerDataMap[reqbrokerdef.ReqBrokerDataMapRespObj] = msgutil.SetResp(rejectdef.LMS_Admin_Service_Reject_HandleSendOTPErr, []byte(appErrDesc), []byte(correctiveAction))
		return -1
	}
	otp := otpObjMap["otp"].(string)
	if len(otp) == 0 {
		appErrDesc := fmt.Sprintf("otp should not be null in reqBrokerDataMap")
		correctiveAction := fmt.Sprintf("Ensure that otp is not a null in reqBrokerDataMap")
		reqBrokerDataMap[reqbrokerdef.ReqBrokerDataMapRespObj] = msgutil.SetResp(rejectdef.LMS_Admin_Service_Reject_HandleSendOTPErr, []byte(appErrDesc), []byte(correctiveAction))
		return -1
	}
	otpObjMapBuffer, _ := json.Marshal(otpObjMap)
	trace.Lg("otpObjMapBuffer[%s]", otpObjMapBuffer)
	reqBuffer := reqbrokerutil.TranslateInternalMsg("VerifyOTP", serviceutil.GetServiceName(), lmsapiengdef.LMSApiEngVersion, json.RawMessage(otpObjMapBuffer))
