...
func Load_1_ServiceAddrInfoTableOracle(serviceModule string, ServiceAddrInfoPtr *[]dbtabdef.ServiceAddrInfoTable, dbContext dbdef.DBContextDef, dberr *string) int {
	var err error
	var rows *sql.Rows
	...
	queryStmt :=
		fmt.Sprintf(`SELECT 
			...
			SERVICE_PORT,
			SERVICE_TIMEOUT
		FROM SERVICE_ADDR_INFO WHERE SERVICE_MODULE='%s'`, serviceModule)
