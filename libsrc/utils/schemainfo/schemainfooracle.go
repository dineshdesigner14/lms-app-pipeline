...
func readModuleFromSchemaOracle(globalSchemaInfoDBID *sql.DB, recordNum string) int {
	var err error
	...
	queryStmt := fmt.Sprintf(
		`SELECT 
			...
			MODULE_NAME,
			SCHEMA_STATUS
		FROM MODULE_SCHEMA_MAP WHERE SCHEMA_RECORD_NUM='%s' AND MODULE_NAME='%s'`, recordNum, moduledef.LMSApiEngModule)
...
func loadGlobalSchemaInfoOracle(globalDBConfig SchemaDBConfig, globalSchemaInfo *[]SchemaInfoTable, globalSchemaInfoDBID *sql.DB) int {
	var lRecordNum sql.NullString
	var lModuleName sql.NullString
	...
	for rows.Next() {
		...
		if readModuleFromSchemaOracle(globalSchemaInfoDBID, lglobalSchemaInfo.RecordNum) >= 0 {
